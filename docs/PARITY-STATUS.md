...
| shell-04 | Search and task navigation | `src/state.rs`, `src/ui.rs` | implemented | G5/G8: persisted selection plus native search screenshot `/tmp/codex-app-gpui-fixture-search.png` |
...
| composer-02 | Composer controls | `src/ui.rs`, `src/state.rs` | implemented | G2/G8: native model/reasoning and attachment/mention screenshots; native picker and app-server attachment serialization are implemented, realtime voice remains staged |
...
| exec-03 | Approvals | `src/state.rs`, `src/protocol.rs`, `src/ui.rs` | implemented | G4/G8: safe decline contract plus native fixture approval screenshot `/tmp/codex-app-gpui-fixture-approved-fixed.png`; both decisions and provider-specific approval variants remain to be exercised |
